...
def process_plate_image(uploaded_image, template_buffer, plate_config, scale_factor=1.0):
    ...
    try:
        ...
        # Load template from buffer and get the correct worksheet
        wb = load_workbook(template_buffer)
        # Try to get the "samples" worksheet first, fallback to active
...
def qr_plate_processor():
    ...
    **Perfect for:** Take pictures on phone → Access from computer → Rename → Process
    ...
    if process_clicked:
        ...
        st.success(f"✅ Template file '{template_file}' found and loaded!")
        ...
        for i, uploaded_image in enumerate(uploaded_images):
            ...
            # Load template directly from file system (like qr_reader.py)
            try:
                ...
                if "samples" in wb.sheetnames:
                    ...
                    ws = wb.active
                    st.warning(f"⚠️ 'samples' worksheet not found in template. Using active worksheet: {ws.title}")
                
                # Create a template buffer for the process_plate_image function
                ...
                st.error(f"❌ Failed to load template file {template_file}: {str(e)}")
                continue
